...
def reinitialize_hyper_head(model: nn.Module):
    ...
    if hasattr(model, "query_encoder") and isinstance(model.query_encoder, Hypencoder):
        ...
        with torch.no_grad():
            ...
            weight_shapes_len = (
                len(query_encoder.weight_shapes)
                if hasattr(query_encoder, "weight_shapes")
                else 0
            )
            bias_shapes_len = (
                len(query_encoder.bias_shapes)
                if hasattr(query_encoder, "bias_shapes")
                else 0
            )
            ...
            for layer_name, init_type in custom_layers.items():
                ...
                expected_len = (
                    weight_shapes_len
                    if layer_name in ["hyper_base_matrices", "weight_query_embeddings"]
                    else min(weight_shapes_len, len(param_list))
                )
            ...
            for layer_name, init_type in custom_layers.items():
                ...
                expected_len = (
                    bias_shapes_len
                    if layer_name in ["hyper_base_vectors", "bias_query_embeddings"]
                    else min(bias_shapes_len, len(param_list))
                )
                if len(param_list) < expected_len:
                    logger.warning(
                        f"{layer_name} has fewer parameters ({len(param_list)}) than expected ({expected_len})"
...
def load_data(data_config: HypencoderDataConfig):
    ...
    if (data_config.training_data_jsonl is None) == (
        ...
        raise ValueError(
            "Must specify either training_data_jsonl or training_huggingface_dataset"
        )
...
def train_model(cfg: HypencoderTrainingConfig):
    ...
    collator = get_collator(
        cfg.data_config, cfg.trainer_config, cfg.logging_config, tokenizer
    )
    ...
    if resume_from_checkpoint is True:
        if not os.path.exists(training_args.output_dir) or not any(
            ["checkpoint" in name for name in os.listdir(training_args.output_dir)]
        ):
