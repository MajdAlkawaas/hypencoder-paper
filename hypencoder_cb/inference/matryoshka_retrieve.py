...
class MatryoshkaHypencoderRetriever(HypencoderRetriever):
    ...
    """
    def __init__(self, matryoshka_dim: int, *args, **kwargs):
        # We need to know which dimension this retriever instance will be responsible for.
        self.matryoshka_dim = matryoshka_dim
        
        # Call the parent's __init__ to handle all the model loading and data setup.
...
def evaluate_matryoshka_checkpoints(
    ...
    specified dimensions, performing data processing only once.
    """
    # =================================================================================
    # --- ONE-TIME DATA DESERIALIZATION & PROCESSING ---
    # =================================================================================
    print("---" * 10)
    print(f"STAGE 1: Deserializing and processing corpus ONCE from: {encoded_item_path}")
    
    preloaded_items_raw = list(load_encoded_items_from_disk(encoded_item_path, target_dtype=dtype))
    
    dtype_torch = dtype_lookup(dtype)
    preloaded_embeddings = torch.stack(
        [torch.tensor(x.representation, dtype=dtype_torch) for x in tqdm(preloaded_items_raw, desc="Stacking Embeddings")]
    )
    preloaded_ids = [x.id for x in tqdm(preloaded_items_raw)]
    preloaded_texts = [x.text for x in tqdm(preloaded_items_raw)]
    del preloaded_items_raw # Free memory
    ...
    # --- Main Loop: Checkpoints -> Dimensions ---
    for i, checkpoint_path in enumerate(checkpoint_paths):
        print("\n" + "#"*70)
        print(f"# LOADING CHECKPOINT {i+1}/{len(checkpoint_paths)}: {checkpoint_path}")
        print("#"*70)

        # --- SURGICAL MODEL LOADING (ONCE PER CHECKPOINT) ---
        ...
        # --- INNER LOOP: Evaluate each Matryoshka dimension for this checkpoint ---
        for dim in matryoshka_dims:
            print("\n" + "="*50)
            print(f"EVALUATING DIMENSION: {dim}")
            
            checkpoint_name = Path(checkpoint_path).name
            # Create a nested output directory: base/checkpoint_name/dimension_name
            output_dir_for_dim = Path(base_output_dir) / checkpoint_name / f"dim_{dim}"
            output_dir_for_dim.mkdir(parents=True, exist_ok=True)
            
            # Initialize our new retriever for this specific dimension
            # It will receive the already loaded model and data tensors
            retriever = MatryoshkaHypencoderRetriever(
                matryoshka_dim=dim,
                model_for_retrieval=local_model, # Pass the composed model
            ...
            retrieve_for_ir_dataset_queries(
                retriever=retriever, ir_dataset_name=ir_dataset_name, output_path=retrieval_file, top_k=top_k
            )
            do_eval_and_pretty_print(
                retrieval_path=retrieval_file, output_dir=output_dir_for_dim / "metrics", ir_dataset_name=ir_dataset_name
            )
